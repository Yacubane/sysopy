...
char* add_timestamp(char* buff) {
  snprintf(timestamp_text_buff, TEXT_BUFF_SIZE, "TIME=%5llu %s",
           millis_since_start(), buff);
...
void* passenger_thread_fun(void* data) {
  ...
  while (1) {
    ...
    while (passenger->state != STATE_IN_TROLLEY_WAITING_FOR_NOTHING &&
           passenger->state != STATE_IN_TROLLEY_WAITING_TO_CLICK_RED)
      pthread_cond_wait(&cond_select_passenger_to_click, &mutex);

    if (passenger->state == STATE_IN_TROLLEY_WAITING_TO_CLICK_RED) {
      snprintf(text_buff, TEXT_BUFF_SIZE,
               "PASSENGER[%d] is clicking red button", passenger->id);
  ...
  snprintf(text_buff, TEXT_BUFF_SIZE, "PASSENGER[%d] is ending it's job",
           passenger->id);
  colorprint(add_timestamp(text_buff), ANSI_COLOR_MAGENTA);
...
void* trolley_thread_fun(void* data) {
  ...
  for (int i = 0; i < rides_num; i++) {
    snprintf(text_buff, TEXT_BUFF_SIZE, "TROLLEY[%d] is opening doors",
             trolley->id);
    ...
    snprintf(text_buff, TEXT_BUFF_SIZE, "TROLLEY[%d] is closing doors",
             trolley->id);
    colorprint(add_timestamp(text_buff), ANSI_COLOR_YELLOW);

    int selected_passenger = rand() % trolley_capacity;
    trolley->passengers[selected_passenger]->state =
        STATE_IN_TROLLEY_WAITING_TO_CLICK_RED;

    for (int i = 0; i < trolley->passengers_count; i++) {
      pthread_cond_broadcast(&cond_select_passenger_to_click);

      if (i == selected_passenger) {
        while (trolley->passengers[i]->state !=
               STATE_IN_TROLLEY_CLICKED_RED_BUTTON)
    ...
    snprintf(text_buff, TEXT_BUFF_SIZE, "TROLLEY[%d] is starting a ride",
             trolley->id);
    colorprint(add_timestamp(text_buff), ANSI_COLOR_YELLOW);
    ...
    snprintf(text_buff, TEXT_BUFF_SIZE, "TROLLEY[%d] is ending a ride",
             trolley->id);
    colorprint(add_timestamp(text_buff), ANSI_COLOR_YELLOW);
    ...
    while (trolley->passengers_count > 0) {
      trolley->passengers[trolley->passengers_count - 1]->state =
          STATE_READY_TO_RELEASE;
      pthread_cond_broadcast(&cond_select_passenger_to_release);

      while (trolley->passengers[trolley->passengers_count - 1]->state !=
             STATE_RELEASED)
  ...
  snprintf(text_buff, TEXT_BUFF_SIZE, "TROLLEY[%d] is ending it's job",
           trolley->id);
  colorprint(add_timestamp(text_buff), ANSI_COLOR_YELLOW);
...
int main(int argc, char* argv[]) {
  if (argc != 5)
    return err(
        "Missing arguments passengers_num, trolleys_num, trolley_capacity, "
        "rides_num",
        -1);
  ...
  for (int i = 0; i < passengers_num; i++) {
    ...
    if (pthread_create(&passengers[i]->thread, NULL, passenger_thread_fun,
                       passengers[i]) < 0)
  ...
  for (int i = 0; i < trolleys_num; i++) {
    ...
    if (pthread_create(&trolleys[i]->thread, NULL, trolley_thread_fun,
                       trolleys[i]) < 0)
