...
pid_t create_fork(char* path,
  ...
  if (child_pid == 0) {
    exit(fork_job(path, refresh_seconds, monitor_seconds, type, cpu_restriction,
                  memory_restriction));
  } else if (child_pid > 0) {
    return child_pid;
  } else {
...
int main(int argc, char* argv[]) {
  ...
  while (fscanf(fd, "%s %d\n", path_buffer, &seconds_buffer) == 2) {
    pid_t child = create_fork(path_buffer, seconds_buffer, monitor_seconds,
                              type, cpu_restriction, memory_restriction);
    if (child < 0)
