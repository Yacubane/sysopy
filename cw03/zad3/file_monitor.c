...
int fork_job(char* path,
             int refresh_seconds,
             int monitor_seconds,
             int type,
             int cpu_restriction,
             int memory_restriction) {
  ...
  if (type == 0) {
    ...
    printf(
        "Raport podprocesu o PID: %d. u_time: %lu [s] %lu [us] s_time: %lu [s] "
        "%ld [us] maxrss %ld\n",
        getpid(), rusage.ru_utime.tv_sec, rusage.ru_utime.tv_usec,
        rusage.ru_stime.tv_sec, rusage.ru_stime.tv_usec, rusage.ru_maxrss);
  } else if (type == 1) {
    struct stat stat_buffer;
    stat(path, &stat_buffer);
    unsigned long last_mod_time = stat_buffer.st_mtime;
    ...
    while (elapsed_seconds < monitor_seconds) {
      ...
      if (last_mod_time != stat_buffer.st_mtime) {
        ...
        if (child_pid == 0) {
          ...
          char new_path[255];
          snprintf(new_path, 255, "%s/%s_%s", "archiwum", basename(path),
                   mtime_buffer);
    ...
    printf(
        "Raport podprocesu dzieci o PID: %d. u_time: %lu [s] %lu [us] s_time: "
        "%lu [s] %ld [us] maxrss %ld. Nie udało się skopiować %d razy.\n",
        getpid(), rusage.ru_utime.tv_sec, rusage.ru_utime.tv_usec,
        rusage.ru_stime.tv_sec, rusage.ru_stime.tv_usec, rusage.ru_maxrss,
        not_copied_num);
  }
