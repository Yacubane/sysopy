...
int main(int argc, char *argv[])
{
    ...
    if(pmax < 1) return create_error("Pmax cannot be less than 1"); 
    if(pmin < 0) return create_error("Pmin cannot be less than 0"); 
    if(pmax < pmin) return create_error("Pmax cannot be less than Pmin"); 
    if(bytes < 0) return create_error("Bytes cannot be less than 0"); 
    ...
    while (1) {
        ...
        snprintf(buffer, 255, "\nPID: %d SEC: %d DATE: %s DATA: %s", getpid(), sec, date_buffer, random_buffer);
