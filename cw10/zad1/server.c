...
void* ping_thread_fun(void* data) {
  while (1) {
    if (mode == MODE_TCP) {
      ...
      for (int j = 0; j < CLIENTS_SIZE; j++)
        if (udp_clients[j]->state != CLIENT_STATE_NULL) {
          timeval_diff(&udp_clients[j]->last_ping_time, &actual_time, &diff);
          if (diff.tv_sec > 5) {
            free(udp_clients[j]->udp_addr);
            udp_clients[j]->state = CLIENT_STATE_NULL;
            colorprintf(ANSI_COLOR_RED, "Removing client[%s] due to inactivity",
                        udp_clients[j]->id);
          }
        }
      ...
      for (int j = 0; j < CLIENTS_SIZE; j++)
        if (udp_clients[j]->state != CLIENT_STATE_NULL) {
          char ch = MSGTYPE_PING;
          sendto(udp_clients[j]->sockid, &ch, 1, 0, udp_clients[j]->udp_addr,
...
int send_request(char* path) {
  ...
  if (mode == MODE_TCP) {
    ...
    if (client == NULL) {
      colorprintf(ANSI_COLOR_RED, "There is no client to send request", path);
      return -1;
    }
    colorprintf(ANSI_COLOR_YELLOW, "Sending request to client[%s]", client->id);
    pthread_mutex_lock(&socket_mutex);
    ...
    pthread_mutex_unlock(&socket_mutex);
    colorprintf(ANSI_COLOR_YELLOW, "Sent request to client[%s]", client->id);
    ...
    request_counter++;
    sendto(client->sockid, buffer, strlen(buffer + 2) + 2, 0, client->udp_addr,
           client->udp_addr_size);
        ...
        if (i == STDIN_FILENO) {
          ...
          if (mode == MODE_TCP) {
            ...
            if (read_from_client_tcp(client) < 0) {
              close(i);
              colorprintf(ANSI_COLOR_RED, "Client [%s] disconnected",
                          client->id);
