...
int parse_command(int index, int argc, char *argv[])
{
    if(!strcmp(argv[index], "generate"))
    {
        ...
        if(errno != 0 || end == argv[index+2])
            return create_error("Error parsing generate, second argument must be a number"); 
        ...
        if(errno != 0 || end == argv[index+3])
            return create_error("Error parsing generate, third argument must be a number"); 
    ...
    else if(!strcmp(argv[index], "sort"))
    {
        ...
        if(errno != 0 || end == argv[index+2])
            return create_error("Error parsing sort, second argument must be a number"); 
        ...
        if(errno != 0 || end == argv[index+3])
            return create_error("Error parsing sort, third argument must be a number");
        ...
        if(!strcmp(lib_flag_string, "sys"))
            ...
            lib_flag = FILES_LIB;
        else return create_error("Error parsing sort, fourth argument must be \"sys\" or \"lib\"");
    ...
    else if(!strcmp(argv[index], "copy"))
    {
        ...
        if(errno != 0 || end == argv[index+3])
            return create_error("Error parsing copy, third argument must be a number"); 
        ...
        if(errno != 0 || end == argv[index+4])
            return create_error("Error parsing copy, fourth argument must be a number");
