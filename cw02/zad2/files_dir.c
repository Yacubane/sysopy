...
char* get_tile_type(struct stat *stat_buffer) 
{
...
int readdir_recursive(struct stat *stat_buffer, int buffer_size, int sign, time_t time) 
{
    ...
    while((file = readdir(dir)) != NULL) {
        ...
        {
            if(lstat(file->d_name, stat_buffer) < 0) return create_error("stat error");
            int print = 1;
            if(sign == -1)
            ...
            if(print == 1) {
                ...
                char cwd_buffer[255];
                if(getcwd(cwd_buffer, 255) == NULL) return create_error("getcwd error");
            ...
            if(S_ISDIR(stat_buffer->st_mode)){
                if(chdir(file->d_name) != 0) return create_error("chdir error 1");
                readdir_recursive(stat_buffer, buffer_size, sign, time);
                if(chdir("..") != 0) return create_error("chdir error 2");
            }
        }
...
int view_dir(char *path, int sign, time_t time) 
{
    ...
    if(chdir(path) != 0) return create_error("chdir error 0");
