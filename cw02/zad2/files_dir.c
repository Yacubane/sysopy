...
int readdir_recursive(struct stat* stat_buffer,
                      int buffer_size,
                      int sign,
                      time_t time) {
  DIR* dir;
  if ((dir = opendir(".")) == NULL)
    return create_error("opendir error");

  struct dirent* file;
  errno = 0;
  while ((file = readdir(dir)) != NULL) {
    if (strcmp(file->d_name, ".") && strcmp(file->d_name, "..")) {
      if (lstat(file->d_name, stat_buffer) < 0)
        return create_error_and_close_dir(dir, "stat error");
      int print = 1;
      if (sign == -1)
        if (stat_buffer->st_mtime / 60 >= time / 60)
          print = 0;
      if (sign == 0)
        if (stat_buffer->st_mtime / 60 != time / 60)
          print = 0;
      ...
      if (print == 1) {
        ...
        char cwd_buffer[255];
        if (getcwd(cwd_buffer, 255) == NULL)
          return create_error_and_close_dir(dir, "getcwd error");

        printf("%10s %11ldB %16s %16s %s/%s\n", get_tile_type(stat_buffer),
               stat_buffer->st_size, atime_buffer, mtime_buffer, cwd_buffer,
               file->d_name);
      }

      if (S_ISDIR(stat_buffer->st_mode)) {
        if (chdir(file->d_name) != 0)
          return create_error_and_close_dir(dir,
                                            "Cannot get forward in files tree");
        readdir_recursive(stat_buffer, buffer_size, sign, time);
        if (chdir("..") != 0)
          return create_error_and_close_dir(dir,
                                            "Cannot get back in files tree");
      }
    }
  }
  if (errno != 0)
...
int view_dir(char* path, int sign, time_t time) {
  ...
  if (chdir(path) != 0)
    return create_error("Cannot change directory to choosen path");
