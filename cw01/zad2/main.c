...
int parse_command(int index, int argc, char* argv[]) {
  if (!strcmp(argv[index], "create_table")) {
    ...
    if (errno != 0 || end == argv[index + 1])
      return create_error(
          "Error parsing create_table, argument must be a number");
    ...
    if (check_argument_size(index, 3, argc) < 0)
      return create_error(
          "Error parsing search_directory, wrong arguments size");
    ...
    if (check_argument_size(index, 1, argc) < 0)
      return create_error(
          "Error parsing remove_data_block, wrong arguments size");
    ...
    if (errno != 0 || end == argv[index + 1])
      return create_error(
          "Error parsing remove_data_block, argument must be a number");
    ...
    if (errno != 0 || end == argv[index + 1])
      return create_error(
          "Error parsing get_data_block, argument must be a number");
    ...
    if (check_argument_size(index, 3, argc) < 0)
      return create_error(
          "Error parsing start_reporting, wrong arguments size");
    ...
    if (errno != 0 || end == argv[index + 3])
      return create_error(
          "Error parsing start_reporting, third argument must be a number");
    ...
    if (check_argument_size(index, 1, argc) < 0)
      return create_error(
          "Error parsing stop_report_timer, wrong arguments size");
