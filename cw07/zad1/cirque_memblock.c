...
int que_put(void* queue_ptr, void* item_ptr) {
  ...
  if (queue->actual_size == queue->max_size) {
    ...
    memcpy(queue->array + elem_num * queue->item_size, item_ptr,
           queue->item_size);
    queue->actual_size += 1;
    return 0;
  }
...
int que_get(void* queue_ptr, void* item_ptr) {
  ...
  if (queue->actual_size == 0) {
    ...
    memcpy(item_ptr, queue->array + queue->head * queue->item_size,
           queue->item_size);
...
int que_peek(void* queue_ptr, int elem_num, void* item_ptr) {
  ...
  if (elem_num >= queue->actual_size) {
    ...
    memcpy(item_ptr, queue->array + elem_num * queue->item_size,
           queue->item_size);
  }
  return 0;
}