...
int send_message(int global_type, char* message) {
  ...
  if (mq_send(server_queue_id, buffer, MAX_STATEMENT_SIZE,
              get_priority_by_type(global_type)) == -1)
...
int main(int argc, char* argv[]) {
  ...
  if ((client_queue_id =
           mq_open(client_path, O_RDONLY | O_CREAT | O_EXCL, 0666, &attr)) < 0)
  ...
  if (mq_send(server_queue_id, buffer, MAX_STATEMENT_SIZE,
              get_priority_by_type(TYPE_INIT)) == -1)
